...
public class EncryptAes : IEncryptAes
{
    private string _defaultPassword = "DCS@DefaultPass^word&QueryString";
    ...
    public string Encrypt(string textString)
    {
        ...
        {
            byte[] passwordBytes;
            byte[] originalBytes = Encoding.UTF8.GetBytes(textString);
            byte[] encryptedBytes = null;

            // Hash the password with SHA256
            passwordBytes = SHA256.Create().ComputeHash(Encoding.UTF8.GetBytes(_defaultPassword));
